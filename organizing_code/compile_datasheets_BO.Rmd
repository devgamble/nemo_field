...
#ignore parse fail warning - one entry 'P' coerced to NA
...
#parse failures due to character strings (7 obs total) entered in date columns - coerced to NA
...
BO_seedct_1 <- read_csv(here::here("data_sheets", "fruit_seed_counts_final", "BO-seed-count_2023-02-15.csv"), skip = 1) %>% 
  ...
  mutate(Plant_ID = as.numeric(Plant_ID)) #three 'NF' entries coerced to NA - ignore parse warning
...
BO_seedmass_1 <- read_csv(here::here("data_sheets", "seed_mass", "BO_Seed_Mass_2023-02-05.csv"), skip = 1) %>%   select(-Donor) %>% slice(1:339)
...
#Weed out duplicates
which(duplicated(select(skel_weights_all, c(Transect, Recipient, Plant_ID))))

skel_BO_duped <- skel_weights_all %>% 
  mutate(duped1 = duplicated(select(skel_weights_all, c(Transect, Recipient, Plant_ID))),
         duped2 = duplicated(select(skel_weights_all, c(Transect, Recipient, Plant_ID)), fromLast = TRUE),
         duped = case_when(duped1|duped2 == TRUE ~ TRUE,
                           TRUE ~ FALSE)) %>% #highlight all duplicates
  select(-c(duped1, duped2)) %>% 
  filter(duped == TRUE)#All dupes appear as true duplicates (exact row copes): N = 10

#Remove duplicates - 210 skel weights total
skel_weights_all2 <- distinct(skel_weights_all)


#Merge with full data set
BO_merged_df5 <- merge(BO_merged_df4, skel_weights_all2, by = c("Transect", "Recipient", "Plant_ID"), all.x = TRUE)
#obs == 3504
...
corollas_BO <- read_csv(here::here("data_sheets", "corolla_measures", "corollameasures_BO_2023-02-28.csv")) %>% select(1, 3, 4, 6, 8, 9, 10) %>% #ignore parse warnings - columns 7 & 14 not retained
...
leaves_BO_ind <- leaves_BO %>% 
  ...
  mutate(SLA = leaf_area_mm2 / leaf_dryweight_mg,
         LMA = 1 / SLA) %>% 
  distinct(Block, Transect, Recipient, Plant_ID, .keep_all = TRUE) #get rid of dupe from W2 BO_195


#Merge - sheet 7b
BO_merged_df7b <- merge(BO_merged_df7, leaves_BO_ind, by = c("Block", "Transect", "Recipient", "Plant_ID"), all.x = TRUE) #measures of individual plants. Total # obs unchanged

# make sure all measurements merged in
BO_NFleaves <- BO_merged_df7b %>% 
  filter(!is.na(leaf_dryweight_mg)) 

#Find mislabeled entry
#which(duplicated(select(BO_NFleaves, c(Block, Transect, Recipient, Plant_ID)))) #Duplicate of W2 BO_295 plant 8 - fixed above
...
```{r}
BO_merged_df_fl <- BO_merged_df7b %>% 
...
BO_merged_df_flB <- BO_merged_df_fl %>% 
...
#Survival measures
BO_merged_df_flC <- BO_merged_df_flB %>% 
...
#Calculate Fecundity 
BO_merged_df_fl2 <- BO_merged_df_flC %>% 
...
Nemo_d13C_BO <- Nemo_d13C %>% 
  ...
  slice(-54) #remove mislabeled entry BO_118 in Tr 5
...
#Merge
BO_merged_df_fl3 <- merge(BO_merged_df_fl2, Nemo_d13_BO2, by = c("Recipient", "Transect", "Plant_ID"), all.x = TRUE)
...
BO_merged_df_fl3b <- merge(BO_merged_df_fl3, Nemo_d13_BO3, by = c("Recipient", "Transect"), all.x = TRUE) 
#Merge by Recipient and Transect only: segment-wide estimates of d13C since some plants were NF

#Check merge
d13C_check_BO <- BO_merged_df_fl3b %>% 
  filter(FitP == TRUE) %>% 
...
skel_weights_seg1 <- merge(BO_GermFlow_3, skel_weights_all2, by = c("Recipient", "Transect", "Plant_ID"), all.x = TRUE) %>% 
  filter(!is.na(AG_biomass_mg)) %>% 
...
BO_merged_df_fl4 <- merge(BO_merged_df_fl3b, skel_weights_SEG, by = c("Block", "Transect", "Sequence", "Donor", "Recipient"), all.x = TRUE)
...
corollas_seg_RAND <- corollas_duped %>% 
  ...
  slice_sample(n = 1) %>%  #select a random row for each  combo of block, transect, recipient
  select(-c(duped, Plant_ID)) 
##326 total obs; 293 obs excluding W block - number of segment measurements there should be
...
#Merge
BO_merged_df_fl5 <- merge(BO_merged_df_fl4, corollas_SEG, by = c("Block", "Transect", "Sequence", "Donor", "Recipient"), all.x = TRUE)
...
BO_mastersheet_v1 <- BO_merged_df_fl5 %>% 
...
BO_mastersheet_v2 <- BO_mastersheet_v1 %>% 
...
#Add Greenhouse cohort info
BO_tray21 <- read_csv(here::here("data_sheets", "tray_positions_2021_year1", "BO_tray_rand.csv"), skip = 1) %>% 
...
#merge
BO_mastersheet_v3 <- left_join(BO_mastersheet_v2, BO_tray21, by = c("Donor", "Recipient")) %>% 
  select(-c(F_plant_notes, Fl_3.11:flr_P, damaged_fruits_F, seeds_weighed_d1:msm_d9, photo_date, notes_corolla, leafarea_notes, leafweight_notes)) %>% #Remove unnecessary columns 
...
write_csv(BO_mastersheet_v3, here::here("data_sheets", "compiled_sheets", "BO_mastersheet_full_2023-11-06.csv"))


## Fitness Plants Only
#
BO_mastersheet_F_mains <-  BO_mastersheet_v3 %>% 
  filter(F_plant == TRUE) #Only main fitness plants

BO_mastersheet_F_all <- BO_mastersheet_v3 %>% 
  filter(any_FitP == TRUE) #main and multiple fitness plants (IDs replicated in watering experiment)


write_csv(BO_mastersheet_F_mains, here::here("data_sheets", "compiled_sheets", "BO_mastersheet_Fitness-mains_2023-11-06.csv"))

write_csv(BO_mastersheet_F_all, here::here("data_sheets", "compiled_sheets", "BO_mastersheet_Fitness-all_2023-11-06.csv"))
