...
**Manual Corrections** to CSV. Location for Seg 270L added.
...
BB_seedmass_2023 <- read_csv(here::here("data_sheets", "2023_year2_data", "fruit_seed_counts", "BB_Seed_Weights_2023-All_Data.csv"), skip = 1) %>% 
  select(-c(4, 7, 8, 9)) %>% #rm closed fruit count
  rename(SegPos = Seg, Plant_ID = Fit_plant, msm_all = 6, 
         closed_fruits_d1 = 7, viable_seeds_d1 = 8, seeds_weighed_d1 = 9, seed_mass_mg_d1 = 10, mean_mass_mg_d1 = 11, 
         closed_fruits_d2 = 12, viable_seeds_d2 = 13, seeds_weighed_d2 = 14, seed_mass_mg_d2 = 15, mean_mass_mg_d2 = 16,
         closed_fruits_d3 = 17, viable_seeds_d3 = 18, seeds_weighed_d3 = 19, seed_mass_mg_d3 = 20, mean_mass_mg_d3 = 21,
         closed_fruits_d4 = 22, viable_seeds_d4 = 23, seeds_weighed_d4 = 24, seed_mass_mg_d4 = 25, mean_mass_mg_d4 = 26,
         closed_fruits_d5 = 27, viable_seeds_d5 = 28, seeds_weighed_d5 = 29, seed_mass_mg_d5 = 30, mean_mass_mg_d5 = 31,
         closed_fruits_d6 = 32, viable_seeds_d6 = 33, seeds_weighed_d6 = 34, seed_mass_mg_d6 = 35, mean_mass_mg_d6 = 36,
         closed_fruits_d7 = 37, viable_seeds_d7 = 38, seeds_weighed_d7 = 39, seed_mass_mg_d7 = 40, mean_mass_mg_d7 = 41,
         closed_fruits_d8 = 42, viable_seeds_d8 = 43, seeds_weighed_d8 = 44, seed_mass_mg_d8 = 45, mean_mass_mg_d8 = 46) %>% 
  filter(!is.na(Recipient)) %>% #Remove empty rows
  select(-47) #remove extra column


#Calculate number of viable seeds 
#Calculate SD of grand mean seed mass

BB_seedmass_2023_B <- BB_seedmass_2023 %>% 
  mutate(ALL_filled_seeds_N = rowSums(across(c(viable_seeds_d1, viable_seeds_d2, viable_seeds_d3, viable_seeds_d4, viable_seeds_d5, viable_seeds_d6, viable_seeds_d7, viable_seeds_d8)), na.rm = TRUE)) %>% #sum viable seed counts across dates (N for NEW)
  rowwise() %>% 
  mutate(sd_msm_all = sd(c_across(c(mean_mass_mg_d1, mean_mass_mg_d2, mean_mass_mg_d3, mean_mass_mg_d4, mean_mass_mg_d5, mean_mass_mg_d6, mean_mass_mg_d7, mean_mass_mg_d8)), na.rm = TRUE)) %>%  #calculate total standard deviation of mean seed mass
  select(Location:Plant_ID, ALL_filled_seeds_N, msm_all, sd_msm_all, everything())
...
#Fix names for BB_205
#(Recall 205_A = Donor BB_217; 205_B = Donor BB_188)
BB_seedmass_2023_B2 <- BB_seedmass_2023_B %>% 
  mutate(Recipient = case_when(Recipient == "BB_205" & SegPos == "111L" ~ "BB_205A",
                               Recipient == "BB_205" & SegPos == "250R" ~ "BB_205A",
                               TRUE ~ Recipient))
 
  
##Merge with fruit/seed count data frame

BB_merged_2.5 <- merge(BB_merged_2, BB_seedmass_2023_B2, by = c("Location", "SegPos", "Gen", "Recipient", "Plant_ID"), all.x = TRUE)
...
BB_merge_check <- BB_merged_2.5 %>% 
  filter(!is.na(msm_all))

BB_merge_check2 <- BB_merged_2.5 %>% 
  filter(!is.na(ALL_filled_seeds_N)) #Should match original seed count Nrows
#(Fixed above) 2 obs missing - check recipient names

## Correct viable seed counts
BB_merged_3 <- BB_merged_2.5 %>% 
...
```{r eval = FALSE}
corollas_BB <- read_csv(here::here("data_sheets", "2023_year2_data", "corolla_leaf_data", "Corolla Diameters 2023 - BB_corollas.csv")) %>% select(-c(4,10, 11)) %>% 
...
write_csv(BB_mastersheet_v1, here::here("data_sheets", "2023_year2_data", "compiled_sheets_2023", "BB_mastersheet_full_Y2_2023-08-07.csv"))
...
write_csv(BB_mastersheet_F_mains, here::here("data_sheets", "2023_year2_data", "compiled_sheets_2023", "BB_mastersheet_Fitness-mains_Y2_2023-08-07.csv"))

write_csv(BB_mastersheet_F_all, here::here("data_sheets", "2023_year2_data", "compiled_sheets_2023", "BB_mastersheet_Fitness-all_Y2_2023-08-07.csv"))
