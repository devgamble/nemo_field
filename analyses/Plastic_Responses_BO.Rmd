...
**Note**: Sample sizes for trait data of different genotypes is ABSOLUTELY abysmal. Focus on population-level responses while controlling for Recipient. Documenting the existence of any GxE effects may be sufficient.  
...
**All data**  
```{r warning = FALSE}
BO_allplants <- read_csv(here::here("data_sheets", "compiled_sheets", "BO_mastersheet_full_2023-02-06.csv")) %>% select(-c(Sequence, FitP, any_FitP, F_multi, F_plant_notes, Fl_3.11:flr_P, seeds_weighed_d1:msm_d9)) %>% 
...
#346 obs vs 246 for fitness plants
...
```{r warning = FALSE}
BO_Fplants <- read_csv(here::here("data_sheets", "compiled_sheets", "BO_mastersheet_Fitness-all_2023-02-06.csv")) %>% select(-c(Sequence, FitP, any_FitP, F_multi, F_plant_notes, Fl_3.11:flr_P, seeds_weighed_d1:msm_d9))

#Standardize traits
BO_plants_z <- BO_Fplants %>% 
  filter(Replicated == "R") %>% #Replicated genotypes only
...
corrplot(cor(na.omit(select(BO_plants_z, c(seed_ct_r, FFD_z, fl_dur_z, AG_biom_z)))), addCoef.col = "white", number.cex = 0.8, method = "square")
...
##### all plants
#FFD
ggplot(data = subset(BO_allplants, !is.na(FFD_c)), aes(x=Treatment, y=FFD_c)) + 
  geom_boxplot(aes(fill = Treatment), color = "black") + labs(x= "Treatment", y = "Flowering Date") +
  theme_classic()
ggplot(data = subset(BO_allplants, !is.na(FFD_c)), aes(x=Treatment, y=FFD_c, group = Recipient)) + 
  geom_line(aes(color = Recipient), linewidth = 1) + 
  labs(x= "Treatment", y = "Flower Duration") +
  theme_classic()

#Fl Duration
ggplot(data = subset(BO_allplants, !is.na(fl_dur_c)), aes(x=Treatment, y=fl_dur_c)) + 
  geom_boxplot(aes(fill = Treatment), color = "black") + labs(x= "Treatment", y = "Flowering Duration") +
  theme_classic()
ggplot(data = subset(BO_allplants, !is.na(fl_dur_c)), aes(x=Treatment, y=fl_dur_c, group = Recipient)) + 
  geom_line(aes(color = Recipient), linewidth = 1) + 
  labs(x= "Treatment", y = "Flower Duration") +
  theme_classic()
...
```{r}
#FFD
t.test(BO_allplants$FFD_c ~ BO_allplants$Treatment)

BO_FFD_aov <- lm(FFD_c ~ Treatment, data = BO_allplants)
TukeyHSD(aov(BO_FFD_aov))


#Flowering duration *******
t.test(fl_dur_c ~ Treatment, data = BO_allplants)

BO_fdur_aov <- lm(fl_duration ~ Treatment, data = BO_allplants)
TukeyHSD(aov(BO_fdur_aov))
...
#Seed Count
t.test(seed_ct_r ~ Treatment, data = BO_plants_z)

#Fruit Count *******
t.test(closed_fruits_F ~ Treatment, data = BO_plants_z)


#Aboveground Biomass
t.test(AG_biom_z ~ Treatment, data = BO_plants_z)

BO_AGb_aov <- lm(AG_biom_z ~ Treatment, data = BO_plants_z)
TukeyHSD(aov(BO_AGb_aov))
...
```{r}
# Simple Linear Model
BO_FFD_m1 <- lm(FFD_c ~ Treatment + Recipient, data = BO_allplants)
...
#
#Mixed models
BO_FFD_m1ma <- lmer(FFD_c ~ Treatment + Recipient + (1|Transect), data = BO_allplants, REML = FALSE)

summary(BO_FFD_m1ma)
r.squaredGLMM(BO_FFD_m1ma)
...
BO_FFD_m1mb <- lmer(FFD_c ~ Treatment + (1|Transect) + (1|Recipient), data = BO_allplants, REML = FALSE)
summary(BO_FFD_m1mb)
r.squaredGLMM(BO_FFD_m1mb) #better r2c than first lmer model
...
AIC(BO_FFD_m1, BO_FFD_m1ma, BO_FFD_m1mb)#Nope

#Other model comparison method: LRT
chi2 <- 1*(summary(BO_FFD_m1mb)$logLik - summary(BO_FFD_m1ma)$logLik) ## 1 indicates df diff b/w models
...
BO_FFD_m1mc <- lmer(FFD_c ~ Treatment + (1|Transect) + (1+Treatment|Recipient), data = BO_allplants, REML = FALSE)
summary(BO_FFD_m1mc)
r.squaredGLMM(BO_FFD_m1mc) #better r2c (model fit), but are we overfitting? AIC suggests maybe

#Model comparisons
AIC(BO_FFD_m1ma, BO_FFD_m1mb, BO_FFD_m1mc)
#chi2 <- 3*(summary(BO_FFD_m1mc)$logLik - summary(BO_FFD_m1ma)$logLik)
#1-pchisq(chi2, 1)
...
BO_FFD_m1m_ge <- lmer(FFD_c ~ Treatment*Recipient + (1|Transect), data = BO_allplants, REML = FALSE)
summary(BO_FFD_m1m_ge)
r.squaredGLMM(BO_FFD_m1m_ge) #Nice r2c

#diagnostics
plot(BO_FFD_m1m_ge)

#Model comparisons
AIC(BO_FFD_m1ma, BO_FFD_m1mb, BO_FFD_m1mc, BO_FFD_m1m_ge)
#LRT
chi2 <- 39*(summary(BO_FFD_m1m_ge)$logLik - summary(BO_FFD_m1ma)$logLik)
1-pchisq(chi2, 1)
...
Conclusion: No plasticity of FFD in response to water availability at BO, but potential for genetic variation in plasticity
...
Rec_BLUPs <- ranef(BO_FFD_m1mc)$Recipient #Random intercepts & slopes model
...
BO_avg_slope_FFD <- summary(BO_FFD_m1mc)$coefficients[2]
geno_data$geno_slopes <- Rec_BLUPs$Treatment1 + BO_avg_slope_FFD

#Genotype Slopes
ggplot(geno_data, aes(Recipient, geno_slopes)) + 
  geom_point(aes(group = Recipient, color = Recipient), size = 3) + 
  ylab("Plasticity (BO-slopes + BLUP slope)") +
  geom_hline(yintercept = 0, lty = 2) +
  theme_classic()
...
```{r}
# Simple linear model
BO_fdur_m1 <- lm(fl_dur_c ~ Treatment + Recipient, data = BO_allplants)
summary(BO_fdur_m1)

#
#G x E effect
BO_fdur_m2 <- lm(fl_dur_c ~ Treatment* Recipient, data = BO_allplants)
summary(BO_fdur_m2)

#
#MM - location
BO_fdur_m1ma <- lmer(fl_dur_c ~ Treatment + Recipient + (1|Transect), data = BO_allplants, REML = FALSE)
summary(BO_fdur_m1ma) 
r.squaredGLMM(BO_fdur_m1ma)

#
#Random intercepts
BO_fdur_m1mb <- lmer(fl_dur_c ~ Treatment + (1|Recipient) + (1|Transect), data = BO_allplants, REML = FALSE)
summary(BO_fdur_m1mb) #SINGULAR
r.squaredGLMM(BO_fdur_m1mb)
#
#Random slopes & intercepts
BO_fdur_m1mc <- lmer(fl_dur_c ~ Treatment + (1+Treatment|Recipient) + (1|Transect), data = BO_allplants, REML = FALSE)
summary(BO_fdur_m1mc) #SINGULAR
r.squaredGLMM(BO_fdur_m1mc)
...
#Model comparison
AIC(BO_fdur_m1,BO_fdur_m2, BO_fdur_m1ma, BO_fdur_m1mb, BO_fdur_m1mc)
