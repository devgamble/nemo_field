...
```{r}
BB_merged_df1 <- merge(BB_GermFlow_3, BB_fruitseed_ct2, by = c("Block", "Location", "Recipient", "Plant_ID"), all.x = TRUE) %>% 
...
BB_mastersheet_v1 <- BB_merged_df_fl2 %>% 
  ...
  arrange(Block, Location, Sequence, Recipient, Plant_ID) %>% #Correct Recipient names - two mat fams with different donors
  mutate(Recipient = case_when(Donor == "BB_217" & Location %in% c("33", "99", "154") ~ "BB_205_A",
                               Donor == "BB_188" & Location %in% c("48", "96", "126") ~ "BB_205_B",
                               TRUE ~ Recipient))

write_csv(BB_mastersheet_v1, here::here("data_sheets", "compiled_sheets", "BB_mastersheet_v1_2022-08-02.csv"))
...
write_csv(BB_mastersheet_Fonly_v1, here::here("data_sheets", "compiled_sheets", "BB_mastersheet_v1_Fitness-only_2022-08-02.csv"))

write_csv(BB_mastersheet_Fonly_v1, here::here("data_sheets", "compiled_sheets", "BB_mastersheet_v2_allFitness-only_2022-08-02.csv"))
