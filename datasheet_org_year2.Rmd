...
AC_template <- AC_plantings_long2 %>% 
  mutate(GenSym = case_when(Gen == "G1" ~ "-",
                            Gen == "G2" ~ "X")) %>% 
  mutate(Date_1 = NA, N_closed_fruit_D1 =NA, N_open_fruit_D1 = NA, N_other_fruit_D1 = NA, N_total_seeds_D1 = NA, N_filled_seeds_D1 = NA,
         Date_2 = NA, N_closed_fruit_D2 = NA, N_open_fruit_D2 = NA, N_other_fruit_D2 = NA, N_total_seeds_D2 = NA, N_filled_seeds_D2 = NA,
         Date_3 = NA, N_closed_fruit_D3 =NA, N_open_fruit_D3 = NA, N_other_fruit_D2 = NA, N_total_seeds_D3 = NA, N_filled_seeds_D3 = NA,
         Date_4 = NA, N_closed_fruit_D4 =NA, N_open_fruit_D4 = NA, N_other_fruit_D3 = NA, N_total_seeds_D4 = NA, N_filled_seeds_D4 = NA,
         Date_5 = NA, N_closed_fruit_D5 =NA, N_open_fruit_D5 = NA, N_other_fruit_D4 = NA, N_total_seeds_D5 = NA, N_filled_seeds_D5 = NA)
...
AC_template2 <- left_join(AC_template, AC_Fitness2, by = c("Sequence", "Transect", "Segment", "position", "Gen", "Recipient")) %>% 
  ...
  select(Transect:Gen, GenSym, Recipient, Fit_plant, GT_closed_fruit, GT_open_fruit, GT_filled_seeds, everything())


#Save csv - Fruit & seed counts
write_csv(AC_template2, here::here("data_sheets", "2023_year2_data", "fruit_seed_counts", "AC_fruit_seed_counts_blank_20230502.csv"), na = "")
...
#Merge in Fitness plant data
BB_template2 <- left_join(BB_template, BB_Fitness2, by = c("Sequence", "Location", "position", "Gen", "Recipient")) %>% 
  mutate(GT_closed_fruit = NA, GT_open_fruit = NA, GT_filled_seeds = NA, notes = NA) %>% #columns for grand totals
  select(Location:Gen, GenSym, Recipient, Fit_plant, GT_closed_fruit, GT_open_fruit, GT_filled_seeds, everything())


#Save csv - Fruit & seed counts
#write_csv(BB_template2, here::here("data_sheets", "2023_year2_data", "fruit_seed_counts", "BB_fruit_seed_counts_blank_20230502.csv"), na = "")
...
#Save csv - Fruit & seed counts
#write_csv(BO_template2, here::here("data_sheets", "2023_year2_data", "fruit_seed_counts", "BO_fruit_seed_counts_blank_20230502.csv"), na = "")
...
#Save csv - Fruit & seed counts
#write_csv(HR_template2, here::here("data_sheets", "2023_year2_data", "fruit_seed_counts", "HR_fruit_seed_counts_blank_20230502.csv"), na = "")
...
AC_skel_weights_23 <- AC_fitness_23 %>% 
  mutate(skel_weight_mg = NA,
         Units = "mg",
         Notes = NA,
         selected = NA) %>% 
  mutate(seg = paste(Segment, position)) %>% 
  select(Transect, seg, Gen, Recipient, Fit_plant, everything(), -Segment, -position, - Sequence)
...
HR_pulvsheet1 <- merge(HR_skel_23, HR_fitness_23, all.x = TRUE, by = c("Transect", "Segment", "position", "Gen", "Recipient"))
...
HR_pulvsheet4 <- HR_pulvsheet3 %>% 
  mutate(`Tube ID` = seq_along(1:n()),
         `Total_weight(capsule+sample)(mg)` = NA, `sample weight (mg)` = NA, capsule_size = "8x5mm", capsule_weight_mg = "31.48", `Tray Name` = NA, Well_ID = NA, `Type of Material` = "plant stem tissue", Analysis = "13C, 15N", Enriched = "No") %>% 
         select(rowN, Transect, Segment, position, Recipient, Gen, Plant_ID, Sample_ID, `Total_weight(capsule+sample)(mg)`, `Tube ID`, `Tube ID`, capsule_size, capsule_weight_mg,  `sample weight (mg)`, everything()) #organize columns
         

##Add in for tray number reps
#`Tray Name` = c(rep(c("Nemo_2023_HR_1", "Nemo_2023_HR_2", "Nemo_2023_HR_3", "Nemo_2023_HR_4", "Nemo_2023_HR_5", "Nemo_2023_HR_6", "Nemo_2023_HR_7", "Nemo_2023_HR_8"), each=96), rep("Nemo_2023_HR_9", 14)) #782 rows
