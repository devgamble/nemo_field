...
AC_fruitprod_3 <- AC_fruitprod_2 %>% 
  ...
  rename(total_fruits_F = Total_fruits_F_NEW)
...
AC_mastersheet_v1 <- AC_merged_df_fl2 %>% 
  select(Block, Transect, Sequence, Donor, Recipient, Replicated, Plant_ID, FitP, Rep_FitP, F_plant, F_multi, F_plant_notes, everything()) %>% 
  arrange(Block, Transect, Sequence, Recipient, Plant_ID)
...
AC_mastersheet_v1_Fonly <-  AC_mastersheet_v1 %>% 
  filter(FitP == TRUE | Rep_FitP == TRUE)


write_csv(AC_mastersheet_v1_Fonly, here::here("data_sheets", "compiled_sheets", "AC_mastersheet_v1_2022-07-26.csv"))
