---
title: "Plasticity Responses to Supplemental Watering - Bodega Bay"
author: "Devin Gamble"
date: "2023-02-14"
...
**Description:**  
This .Rmd contains analyses of data from the supplemental watering experiment in year 1 (2021-2022) of the *Nemophila menziesii* field experiment. Plasticity is estimated as the effect of watering (y/n) on phenotypic traits in four populations.


**Note**: Sample sizes for trait data of different genotypes is abysmal. Focus on population-level responses while controlling for Recipient. Documenting the existence of any GxE effects may be sufficient.  
...
**All data**  
```{r warning = FALSE}
BB_allplants <- read_csv(here::here("data_sheets", "compiled_sheets", "BB_mastersheet_full_2023-02-06.csv")) %>% select(-c(Sequence, Co., FitP, any_FitP, F_multi, F_plant_notes, Fl_2.25:flr_P, seeds_weighed_d1:msm_d9)) %>% 
...
```{r warning = FALSE}
BB_Fplants <- read_csv(here::here("data_sheets", "compiled_sheets", "BB_mastersheet_Fitness-all_2023-02-06.csv")) %>% select(-c(Sequence, Co., FitP, any_FitP, F_multi, F_plant_notes, Fl_2.25:flr_P, seeds_weighed_d1:msm_d9))

#Standardize traits
BB_plants_z <- BB_Fplants %>% 
  filter(Replicated == "R") %>% #Replicated genotypes only
...
corrplot(cor(na.omit(select(BB_plants_z, c(seed_ct_r, FFD_z, fl_dur_z, AG_biom_z)))), addCoef.col = "white", number.cex = 0.8, method = "square")
...
##### all plants
#FFD
ggplot(data = subset(BB_allplants, !is.na(FFD_c)), aes(x=Treatment, y=FFD_c)) + 
  geom_boxplot(aes(fill = Treatment), color = "black") + labs(x= "Treatment", y = "Flowering Date") +
  theme_classic()
ggplot(data = subset(BB_allplants, !is.na(FFD_c)), aes(x=Treatment, y=FFD_c, group = Recipient)) + 
  geom_line(aes(color = Recipient), linewidth = 1) + 
  labs(x= "Treatment", y = "Flower Duration") +
  theme_classic()

#Fl Duration
ggplot(data = subset(BB_allplants, !is.na(fl_dur_c)), aes(x=Treatment, y=fl_dur_c)) + 
  geom_boxplot(aes(fill = Treatment), color = "black") + labs(x= "Treatment", y = "Flowering Duration") +
  theme_classic()
ggplot(data = subset(BB_allplants, !is.na(fl_dur_c)), aes(x=Treatment, y=fl_dur_c, group = Recipient)) + 
  geom_line(aes(color = Recipient), linewidth = 1) + 
  labs(x= "Treatment", y = "Flower Duration") +
  theme_classic()
...
```{r}
#FFD
t.test(BB_allplants$FFD_c ~ BB_allplants$Treatment)

BB_FFD_aov <- lm(FFD_c ~ Treatment, data = BB_allplants)
TukeyHSD(aov(BB_FFD_aov))


#Flowering duration *******
t.test(fl_dur_c ~ Treatment, data = BB_allplants)

BB_fdur_aov <- lm(fl_duration ~ Treatment, data = BB_allplants)
TukeyHSD(aov(BB_fdur_aov))
...
#Seed Count
t.test(seed_ct_r ~ Treatment, data = BB_plants_z)

#Fruit Count *******
t.test(closed_fruits_F ~ Treatment, data = BB_plants_z)


#Aboveground Biomass
t.test(AG_biom_z ~ Treatment, data = BB_plants_z)

BB_AGb_aov <- lm(AG_biom_z ~ Treatment, data = BB_plants_z)
TukeyHSD(aov(BB_AGb_aov))
...
```{r}
# Simple Linear Model
BB_FFD_m1 <- lm(FFD_c ~ Treatment + Recipient, data = BB_allplants)
...
#
#Mixed models
BB_FFD_m1ma <- lmer(FFD_c ~ Treatment*Recipient + seed_ct + (1|Location), data = BB_allplants, REML = FALSE)

summary(BB_FFD_m1ma)
r.squaredGLMM(BB_FFD_m1ma)
...
BB_FFD_m1mb <- lmer(FFD_c ~ Treatment + seed_ct + (1|Location) + (1|Recipient), data = BB_allplants, REML = FALSE)
summary(BB_FFD_m1mb)
r.squaredGLMM(BB_FFD_m1mb) #better r2c than first lmer model
...
AIC(BB_FFD_m1, BB_FFD_m1ma, BB_FFD_m1mb)#Nope
...
chi2 <- 1*(summary(BB_FFD_m1mb)$logLik - summary(BB_FFD_m1ma)$logLik) ## 1 indicates df diff b/w models
...
BB_FFD_m1mc <- lmer(FFD_c ~ Treatment + (1|Location) + (1+Treatment|Recipient), data = BB_allplants, REML = FALSE)
summary(BB_FFD_m1mc)
r.squaredGLMM(BB_FFD_m1mc) #better r2c (model fit), but are we overfitting? AIC suggests maybe

#Model comparisons
AIC(BB_FFD_m1ma, BB_FFD_m1mb, BB_FFD_m1mc)
#chi2 <- 3*(summary(BB_FFD_m1mc)$logLik - summary(BB_FFD_m1ma)$logLik)
#1-pchisq(chi2, 1)
...
BB_FFD_m1m_ge <- lmer(FFD_c ~ Treatment*Recipient + (1|Location), data = BB_allplants, REML = FALSE)
summary(BB_FFD_m1m_ge)
r.squaredGLMM(BB_FFD_m1m_ge) #Nice r2c

#diagnostics
plot(BB_FFD_m1m_ge)

#Model comparisons
AIC(BB_FFD_m1ma, BB_FFD_m1mb, BB_FFD_m1mc, BB_FFD_m1m_ge)
#LRT
chi2 <- 39*(summary(BB_FFD_m1m_ge)$logLik - summary(BB_FFD_m1ma)$logLik)
1-pchisq(chi2, 1)
...
Conclusion: No plasticity of FFD in response to water availability at BB, but potential for genetic variation in plasticity
...
Rec_BLUPs <- ranef(BB_FFD_m1mc)$Recipient #Random intercepts & slopes model
...
BB_avg_slope_FFD <- summary(BB_FFD_m1mc)$coefficients[2]
geno_data$geno_slopes <- Rec_BLUPs$Treatment1 + BB_avg_slope_FFD

#Genotype Slopes
ggplot(geno_data, aes(Recipient, geno_slopes)) + 
  geom_point(aes(group = Recipient, color = Recipient), size = 3) + 
  ylab("Plasticity (BB-slopes + BLUP slope)") +
  geom_hline(yintercept = 0, lty = 2) +
  theme_classic()
...
```{r}
# Simple linear model
BB_fdur_m1 <- lm(fl_dur_c ~ Treatment + Recipient, data = BB_allplants)
summary(BB_fdur_m1)

#
#G x E effect
BB_fdur_m2 <- lm(fl_dur_c ~ Treatment* Recipient, data = BB_allplants)
summary(BB_fdur_m2)

#
#MM - location
BB_fdur_m1ma <- lmer(fl_dur_c ~ Treatment + Recipient + (1|Location), data = BB_allplants, REML = FALSE)
summary(BB_fdur_m1ma) 
r.squaredGLMM(BB_fdur_m1ma)

#
#Random intercepts
BB_fdur_m1mb <- lmer(fl_dur_c ~ Treatment + (1|Recipient) + (1|Location), data = BB_allplants, REML = FALSE)
summary(BB_fdur_m1mb) #SINGULAR
r.squaredGLMM(BB_fdur_m1mb)
#
#Random slopes & intercepts
BB_fdur_m1mc <- lmer(fl_dur_c ~ Treatment + (1+Treatment|Recipient) + (1|Location), data = BB_allplants, REML = FALSE)
summary(BB_fdur_m1mc) #SINGULAR
r.squaredGLMM(BB_fdur_m1mc)
...
#Model comparison
AIC(BB_fdur_m1,BB_fdur_m2, BB_fdur_m1ma, BB_fdur_m1mb, BB_fdur_m1mc)

```

Best F-dur model: Mixed-model with Location as random effect. Similar AIC to simple model but better R2(c).

Conclusion: among-genotype plasticity not detected
...
```{r}
# Simple linear model
BB_AGb_m1 <- lm(AG_biom_z ~ Treatment + Recipient, data = BB_plants_z)
summary(BB_AGb_m1) #bad

#
#G x E effect
BB_AGb_m2 <- lm(AG_biom_z ~ Treatment* Recipient, data = BB_plants_z)
summary(BB_AGb_m2)

#
#MM - location
BB_AGb_m1ma <- lmer(AG_biom_z ~ Treatment + Recipient + (1|Location), data = BB_plants_z, REML = FALSE)
summary(BB_AGb_m1ma) #SINGULAR
r.squaredGLMM(BB_AGb_m1ma)
##More complex models also singular, not a good idea with such low sample sizes

#Model comparison
AIC(BB_AGb_m1,BB_AGb_m2, BB_AGb_m1ma)

```


Conclusion: No observable plasticity in AG Biomass in BB to awter availability.



#### Effect of Watering on Trait means and varainace  
...
#### Multivaraite mixed models??



#### Selection on Plasticity  

Use either genotypic slopes or PIs as the unit of analysis

**FFD**  


```{r}
## Interaction between Trait & Treatment on Fecundity

BB_F_lm1 <- lm(seed_ct_r ~ FFD_z * Treatment + Recipient, data = BB_plants_z)
summary(BB_F_lm1)


```

Extract and regress Genotype BLUP slopes from best models 
```{r}
...
BB_fdur_lm1 <- lm(seed_ct_r ~ fl_dur_z + Recipient, data = BB_plants_z)
summary(BB_fdur_lm1)

## Flowering duration increased fecundity, on average. Additional watering increased flowering duration. While there was no relationship between seed count and fl duration, this suggests that plastic responses to increased water availability in flowering duration are adaptive.
